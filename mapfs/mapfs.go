...
func (fs *mapFileSystem) setEffectiveIDs(euid, egid int) (ouid, ogid int, err error) {
	...
	log.Printf("set-effective-ids old uid: %d, gid %d, desired uid %d, gid%d, actual uid %d gid %d\n", ouid, ogid, euid, egid, fs.syscall.Geteuid(), fs.syscall.Getegid())
	return ouid, ogid, nil
}

func (fs *mapFileSystem) OnMount(nodeFs *pathfs.PathNodeFs) {
	ouid, ogid, err := fs.setEffectiveIDs(int(fs.uid), int(fs.gid))
	defer fs.setEffectiveIDs(ouid, ogid)
	if err != nil {
		log.Fatal(err)
	}
...
func (fs *mapFileSystem) GetAttr(name string, context *fuse.Context) (a *fuse.Attr, code fuse.Status) {
	ouid, ogid, _ := fs.setEffectiveIDs(int(fs.uid), int(fs.gid))
	defer fs.setEffectiveIDs(ouid, ogid)
